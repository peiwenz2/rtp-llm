...
torch::Tensor narrowTokenAlignedTensor(
    ...
    RTP_LLM_CHECK_WITH_INFO(tensor.dim() > 0 && static_cast<size_t>(tensor.size(0)) == total_tokens,
                            "chunk Prefill requires token-aligned %s: first_dim=%ld total_tokens=%ld",
                            field_name,
...
// Convert any Python chunk-round object exposing a ``slices`` iterable into
// the model-neutral C++ carrier. The planner remains authoritative for round
// scheduling; the executor only consumes packed offsets and lengths.
PrefillChunkRound parsePyPrefillChunkRound(const py::object& round_plan) {
    PrefillChunkRound round;
    const py::object  slices = round_plan.attr("slices");
    for (const auto& slice_obj : slices) {
        PrefillChunkSlice slice;
...
GptModelInputs MtpExecutor::makePrefillRoundInput(const GptModelInputs& full_inputs,
    ...
    for (const auto& slice : round.slices) {
        RTP_LLM_CHECK_WITH_INFO(slice.original_batch_idx >= 0
                                ...
                                "MTP chunk Prefill round contains an invalid slice");
...
void MtpExecutor::setPrefillChunkCacheStorePublishPlan(GptModelInputs&          chunk_input,
    ...
    RTP_LLM_CHECK_WITH_INFO(static_cast<size_t>(chunk_input.input_lengths.numel()) == round.slices.size()
                            ...
                            "MTP chunk cache-store range size mismatch: input=%ld prefix=%ld slices=%zu",
...
void MtpExecutor::shiftRoundComboTokens(GptModelInputs&         chunk_input,
    ...
    for (const auto& slice : round.slices) {
        RTP_LLM_CHECK_WITH_INFO(static_cast<size_t>(slice.source_start) + 1 + slice.new_length <= total_tokens,
                                "MTP chunk Prefill draft shift overruns the packed token count");
...
void MtpExecutor::runChunkPrefillRound(ChunkPrefillContext& hook, const PrefillChunkRound& round, bool is_last) {
    ...
    for (const auto& slice : round.slices) {
        RTP_LLM_CHECK_WITH_INFO(slice.original_batch_idx >= 0
                                ...
                                "MTP chunk Prefill slice has invalid request index %d",
                                slice.original_batch_idx);
        if (!slice.terminal) {
        ...
        RTP_LLM_CHECK_WITH_INFO(!hook.terminal_seen[request_idx],
                                "MTP chunk Prefill request %zu became terminal more than once",
                                request_idx);
    ...
    if (is_last) {
        RTP_LLM_CHECK_WITH_INFO(
            ...
            "MTP chunk Prefill final planner round left a request unterminated");
    }
...
absl::Status MtpExecutor::prefillStep(const std::list<GenerateStreamPtr>& streams,
    ...
    if (model_input.pd_separation) {
        pd_request_id         = model_input.request_id.clone();
        pd_request_separation = model_input.request_pd_separation.clone();
        pd_cache_keys         = model_input.cache_keys.clone();
    }
    ...
    if (chunked_mtp_prefill) {
        ...
        RTP_LLM_CHECK_WITH_INFO((tp_rank_ == 0 && !streams.empty()) || (tp_rank_ != 0 && streams.empty()),
                                ...
                                streams.size(),
                                tp_rank_);
        RTP_LLM_CHECK_WITH_INFO(!cp_enabled, "MTP chunk Prefill does not support Prefill CP");
        ...
        // The current Kimi K3 caller shares one --seq_size_per_block between
        // target and draft. Express that requirement as a generic executor
        // contract rather than baking model-specific block arithmetic here.
        ...
        RTP_LLM_CHECK_WITH_INFO(target_cache_cfg.seq_size_per_block == draft_cache_cfg.seq_size_per_block,
                                "MTP chunk Prefill requires matching target/draft seq_size_per_block: "
                                "target=%zu draft=%zu",
        ...
        RTP_LLM_CHECK_WITH_INFO(py_model != nullptr && draft_model_ != nullptr,
                                "MTP chunk Prefill requires a PyWrappedModel target and a draft model");
        ...
        struct PrefillRoundHookGuard {
            ...
            ~PrefillRoundHookGuard() {
                model->setChunkPrefillRoundHook({});
            }
        } hook_guard{py_model};
        ...
        RTP_LLM_CHECK_WITH_INFO(chunk_hook.terminal_round.slices.size() == chunk_hook.terminal_seen.size(),
                                "MTP chunk Prefill terminal request count mismatch: got=%zu expected=%zu",
...
absl::Status MtpExecutor::decodeStepTargetOnly(const std::list<GenerateStreamPtr>& streams,
    ...
    // Release both models' transient workspaces before the target forward;
    // otherwise the tightly provisioned B300 deployment can fail a small
    // temporary allocation even though speculative decode itself fits.
    releaseAllModelBuffers();
...
void MtpExecutor::prepareStreams(const std::list<GenerateStreamPtr>& streams,
    ...
    for (auto& stream : streams) {
        ...
        if (stream->isContextStream()) {
            ...
            stream->setScoreLen(stream->forceDisableSpRun() ? 1 : propose_step_ + 1);
...
absl::Status MtpExecutor::process(const std::list<GenerateStreamPtr>& streams, int64_t schedule_time_us) {
    ...
    // Do not introduce a standalone TP collective here: process() invocation
    // counts are not guaranteed to match across ranks. A mismatched broadcast
    // leaves worker ranks spinning and prevents their CacheStore clients from
    // draining sockets. Log local state first; request mode will be propagated
    // through an existing rank-synchronised model-input path.
    // prepare streams
...
void MtpExecutor::draftModelDecode(GptModelInputs&             model_input,
...

    {
...
#if USING_CUDA
        // Diagnostic correctness path: temporarily bypass the fused token/
        // metadata packer to isolate target-verify token-position corruption.
        ...
        // Target verify changes the packed context from the original prompt
        // length to ``propose_step + 1`` tokens per request.  Keep the optional
        // host mirrors consistent with that new layout.  Kimi3 deliberately
        // uses these mirrors to validate cu_seqlens without a D2H copy; leaving
