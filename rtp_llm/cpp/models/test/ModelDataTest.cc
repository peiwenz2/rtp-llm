...
TEST_F(ModelDataTest, testDefaultPrefillCudaGraphBucketsAreClippedToModelLimit) {
    ...
    EXPECT_EQ(defaultPrefillCudaGraphCaptureSeqLens(64).back(), 64);
    EXPECT_EQ(defaultPrefillCudaGraphCaptureSeqLens(160).back(), 160);
    EXPECT_EQ(defaultPrefillCudaGraphCaptureSeqLens(4096).back(), 160);
