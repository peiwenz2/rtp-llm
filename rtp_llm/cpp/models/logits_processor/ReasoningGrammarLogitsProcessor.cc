...
ReasoningGrammarLogitsProcessor::ReasoningGrammarLogitsProcessor(std::shared_ptr<RtpGrammarMatcher> matcher,
    ...
    think_info_.reasoning_stop_token_ids = std::move(reasoning_stop_token_ids);
    ...
    if (eos_token_id >= 0 && eos_token_id <= std::numeric_limits<int>::max()) {
        think_info_.reasoning_stop_token_ids.push_back(static_cast<int>(eos_token_id));
    }
    if (!end_think_token_ids.empty()) {
        auto& stop_token_ids = think_info_.reasoning_stop_token_ids;
        stop_token_ids.erase(
            std::remove(stop_token_ids.begin(), stop_token_ids.end(), end_think_token_ids.front()),
            stop_token_ids.end());
    }
    std::sort(think_info_.reasoning_stop_token_ids.begin(), think_info_.reasoning_stop_token_ids.end());
    think_info_.reasoning_stop_token_ids.erase(
        std::unique(think_info_.reasoning_stop_token_ids.begin(), think_info_.reasoning_stop_token_ids.end()),
        think_info_.reasoning_stop_token_ids.end());
...
void ReasoningGrammarLogitsProcessor::updateStatus(const torch::Tensor& new_tokens, int32_t num_new_tokens) {
    ...
    for (int32_t i = 0; i < num_new_tokens; ++i) {
        ...
        if (think_info_.consumePendingForcedToken(token_id)) {
            continue;
        }

        const bool token_belongs_to_reasoning = think_info_.isActive();
        think_info_.advanceToken(token_id);
        if (token_belongs_to_reasoning) {
            continue;
        }
...
int ReasoningGrammarLogitsProcessor::tryAcceptAndFillBitmask(const SpecLogitsProcessorRequest& request) {
    ...
    auto fill_grammar_row = [&](int32_t* row) {
        ...
        clearTokenFromBitmask(row, W, think_state.beginBoundaryTokenToMask());
        clearTokenFromBitmask(row, W, think_state.endBoundaryTokenToMask());
    };

    auto fill_row = [&](int32_t* row) {
        applyThinkSpecRowMask(row, W, think_state);
    ...
    for (int offset = 0; offset <= P; ++offset) {
        ...
        if (token_belongs_to_grammar) {
            ...
            think_state.advanceToken(draft_token);
        } else {
            think_state.advanceToken(draft_token);
        }
    }
...
bool ReasoningGrammarLogitsProcessor::applyReasoningOrGrammarMaskLocked(const SamplerInputs& inputs, size_t batch_idx) {
    ...
    switch (think_info_.process_state) {
        ...
        case ThinkProcessState::IN_THINK: {
            if (think_info_.transitionToAfterThinkIfClosed()) {
                return applyGrammarMaskLocked(logits);
            }
            const bool boundary_in_progress = think_info_.closeInProgress();
            ...
            if (boundary_in_progress || budget_exhausted) {
                ...
                return forceThinkEndTokenLocked(logits, boundary_in_progress ? "boundary" : "budget");
            }
            maskToken(logits, think_info_.beginBoundaryTokenToMask());
            maskReasoningStopTokens(logits);
            return true;
        }
        case ThinkProcessState::CLOSING_THINK: {
            if (think_info_.transitionToAfterThinkIfClosed()) {
            ...
            if (forceThinkEndTokenLocked(logits, "boundary")) {
                return true;
            }
            maskToken(logits, think_info_.beginBoundaryTokenToMask());
            maskReasoningStopTokens(logits);
...
bool ReasoningGrammarLogitsProcessor::applyGrammarMaskLocked(const torch::Tensor& logits) {
    ...
    if (!matcher_->fillBitmask(&dl, 0)) {
        maskToken(logits, think_info_.beginBoundaryTokenToMask());
        maskToken(logits, think_info_.endBoundaryTokenToMask());
        return false;
    }
...
bool ReasoningGrammarLogitsProcessor::forceThinkEndTokenLocked(const torch::Tensor& logits, const char* trigger) {
    ...
    RTP_LLM_LOG_INFO("force think boundary: trigger=%s token=%d progress=%zu/%zu",
                     ...
                     token_id,
                     next_token_idx + 1,
                     think_info_.end_think_token_ids.size());
    forceToken(logits, token_id);
    if (reported_error_.load(std::memory_order_relaxed)) {
        return false;
    }

    think_info_.precommitForcedToken(token_id);
    return true;
}

void ReasoningGrammarLogitsProcessor::maskReasoningStopTokens(const torch::Tensor& logits) {
    for (const int token_id : think_info_.reasoning_stop_token_ids) {
