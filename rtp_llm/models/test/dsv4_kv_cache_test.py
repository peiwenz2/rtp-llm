...
class Dsv4MoeConfigTest(TestCase):
    ...
    def test_shared_expert_count_rejects_unsupported_values(self):
        for value in (True, False, 0.5, 1.0, "1", None, -1, 2):
            with self.subTest(value=value), self.assertRaisesRegex(
                ValueError, r"supported integer value \(0 or 1\)"
            ):
        ...
        for n_shared_experts in (0, 1):
            with self.subTest(n_shared_experts=n_shared_experts):
                ...
                if n_shared_experts == 0:
                    ...
                    self.assertEqual(
                        {ckpt.name for ckpt in layer_by_name[W.v4_shared_w2_w].weights},
                        {"layers.{i}.ffn.shared_experts.w2.weight"},
                    )

    def test_generic_public_moe_strategy_falls_back_to_dsv4_auto(self):
        ...
        self.assertIn("not in the DSV4 strategy namespace", "\n".join(logs.output))
    ...
    def test_public_strategy_reaches_v4_block_and_moe_selection(self):
        ...
        class DummyGate(DummyModule):
            def __init__(self, *args, **kwargs):
                super().__init__()
                self.route_scale = float(args[5])
        ...
        cases = (
            ("mega_moe", 0, MegaMoEStrategy),
            ("mega_moe_se", 1, MegaMoESEStrategy),
        )
        for strategy, n_shared_experts, expected_cls in cases:
            with self.subTest(strategy=strategy), patch.dict(
                ...
                with patch(
                    ...
                    "rtp_llm.models_py.modules.dsv4.moe.moe_layer.Gate",
                    DummyGate,
                ), patch.object(
                    expected_cls, "can_handle", return_value=True
                ), patch.object(
                    expected_cls, "setup_weights", return_value=None
                ), patch.object(
                    expected_cls,
                    "can_use_gate_pack_static",
                    return_value=False,
                ):
                    block = Block(**kwargs)
                ...
                self.assertIsInstance(block.ffn._strategy, expected_cls)
