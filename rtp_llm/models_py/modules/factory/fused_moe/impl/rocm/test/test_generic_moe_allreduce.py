...
def _build_batched_moe(rank, parallelism_config):
    ...
    if fused_moe.topk_ids_dtype is not torch.int32:
        raise AssertionError(
            f"expected int32 top-k IDs, got {fused_moe.topk_ids_dtype}"
        )
...
def _batched_inputs(case, device):
    ...
    logits[tokens, (tokens + case) % 4] = 3.0
    logits[tokens, 4 + (tokens + case + 1) % 4] = 2.0
...
def _run_batched_eager_and_graph(rank):
    ...
    topk_ids = torch.empty(hidden.size(0), 2, dtype=torch.int32, device=device)
    ...
    with patch.object(rocm_rccl, "_is_hipgraph_capture_active", return_value=True):
