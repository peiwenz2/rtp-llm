...
class DeepGemmMaskedExecutorV2CudaGraphTest(unittest.TestCase):
    ...
    def _make_inputs(
        self, offset: int, skewed_routing: bool = False
    ) -> tuple[torch.Tensor, ...]:
        ...
        if skewed_routing:
            topk_ids = torch.empty(
                (self.NUM_TOKENS, self.TOP_K), device="cuda", dtype=torch.int64
            )
            topk_ids[:48] = torch.tensor([0, 1], device="cuda")
            topk_ids[48:] = torch.tensor([1, 2], device="cuda")
        else:
