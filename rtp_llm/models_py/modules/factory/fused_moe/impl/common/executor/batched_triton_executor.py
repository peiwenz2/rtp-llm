...
class BatchedTritonExperts(FusedMoeExpertExecutor):
    """Triton experts on the batched (E, rows, hidden) layout that
    BatchedDataRouter produces."""
    ...
    def execute(
        ...
        if apply_router_weight_on_input:
            raise NotImplementedError(
                ...
                "finalize() then skips its own multiply and the router weights "
                "would be dropped entirely"
            )

        expert_x = payload.expert_x
        if expert_x.dim() != 3 or expert_x.size(2) != self.w1.size(2):
            raise ValueError(
                f"Expected expert_x shape (E, rows, {self.w1.size(2)}), "
                f"got {tuple(expert_x.shape)}"
            )
        if not expert_x.is_contiguous():
            raise ValueError("expert_x must be contiguous")

        meta = payload.expert_tokens_meta
        if meta is None or meta.expert_num_tokens is None:
            raise ValueError("expert_tokens_meta.expert_num_tokens is required")
        expert_num_tokens = meta.expert_num_tokens

        E = self.local_num_experts
        N = self.w1.size(1)
        # Every Triton grid axis below derives from E; a mismatch writes OOB.
        if (
            expert_x.size(0) != E
            or expert_num_tokens.dim() != 1
            or expert_num_tokens.size(0) != E
        ):
            raise ValueError(
                f"Expected {E} local experts, got expert_x "
                f"{tuple(expert_x.shape)} and counts "
