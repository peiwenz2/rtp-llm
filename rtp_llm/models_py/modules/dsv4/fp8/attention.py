...
from rtp_llm.models_py.modules.dsv4.fp8._kv_cache_utils import (
    pool_physical_tokens_per_block as _dsv4_pool_owner_tokens_per_block,
)
from rtp_llm.models_py.modules.dsv4.fp8._kv_cache_utils import (
    require_pool_tokens_per_block as _dsv4_pool_tokens_per_block,
)
from rtp_llm.models_py.modules.dsv4.kv_cache_utils import kv_tag_for_compress_ratio
...
class AttentionFP8(nn.Module):
    def __init__(
        ...
        # ``self._kv_cache.get_layer_cache(layer_id, tag).kv_cache_base``
    ...
    def _pool_view_3d_fp8(self, attn_type: str) -> Optional[torch.Tensor]:
    ...
    def _pool_entries_per_block(self, attn_type: str) -> int:
        """Derive ``entries_per_block`` from the framework pool tensor for
        this layer + cache tag.  Returns 0 if pool unavailable."""
        if self._kv_cache is None:
            return 0
        spec = self._pool_spec.get(attn_type)
        if spec is None:
    ...
    def _set_compressor_pool_context(self) -> None:
        ...
        if self.compressor is not None:
            ...
            kv_owner_tpb = _dsv4_pool_owner_tokens_per_block(self._kv_cache, kv_at)
            state_tpb = (
    ...
    def _forward_decode_hca(
        ...
        from rtp_llm.models_py.modules.dsv4.kv_cache_utils import HCA_KV, HCA_STATE
        ...
        *,
        ...
        if kv_cache_sharded and ratio > 0 and self._kv_cache is not None:
            kv_owner_tpb = _dsv4_pool_owner_tokens_per_block(
                self._kv_cache, kv_tag_for_compress_ratio(ratio)
            )
