...
def _worker(
    ...
    try:
        ...
        assert isinstance(mega._strategy, MegaMoEStrategy)
        assert isinstance(reference._strategy, LocalLoopStrategy)
        if include_shared:
            assert not mega._skip_shared_expert
            assert mega.shared_experts is not None
            assert mega._shared_executor is not None
        else:
            assert mega._skip_shared_expert
            assert mega.shared_experts is None
            assert mega._shared_executor is None
...
def _se_worker(
    ...
    try:
        ...
        global_weights = _make_global_weights(device, include_shared=True)
        local_start = rank * (_EXPERTS // world_size)
        local_end = local_start + (_EXPERTS // world_size)
        mega_se_weights = {
            W.v4_router_w: global_weights[W.v4_router_w],
        ...
        assert isinstance(mega_se._strategy, MegaMoESEStrategy)
        assert isinstance(reference._strategy, LocalLoopStrategy)
        assert mega_se._routed_includes_shared
        assert mega_se._skip_shared_expert
        assert mega_se.shared_experts is None
        assert mega_se._shared_executor is None
        assert reference.shared_experts is not None
        assert reference._shared_executor is not None
...
class RoutedOnlyMegaMoEExecutionTest(unittest.TestCase):
    ...
    def test_two_rank_default_se_handles_empty_rank(self):
        self._spawn(_se_worker, (8, 0), False)

    def test_two_rank_default_se_matches_standalone_and_cuda_graph(self):
