...
def _make_layer_weights(
    ...
    *,
    ...
    def fp4_scale(out_dim: int, in_dim: int) -> torch.Tensor:
        ...
        # collapse to the same scale. UE8M0 bytes 108..110 are nonuniform
        # powers of two in a model-like range that keeps SwiGLU conditioned.
        return torch.randint(
            108,
            111,
...
class GroupedMoEExecutionTest(unittest.TestCase):
    ...
    def test_grouped_matches_local_loop_and_handles_empty_rank(self):
        ...
        self.assertIsInstance(grouped._strategy, GroupedFP4Strategy)
        self.assertIsInstance(local._strategy, LocalLoopStrategy)
        for moe in (grouped, local):
            self.assertFalse(moe._skip_shared_expert)
            self.assertIsNotNone(moe.shared_experts)
            self.assertIsNotNone(moe._shared_executor)
